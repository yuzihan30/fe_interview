...
1. 浏览器和 node 事件队列区别
   ...
   node 有 6 个宏任务和 6 个微任务队列，按优先级执行，一个宏任务队列的全部执行完成后，才会去清空该宏任务队列搭配的微任务队列；6 个宏任务按优先级分阶段执行，完成一个循环，常用宏任务有 Timers、Poll(Io 事件)、Check(setImmediate), 微任务有 process.nextTick,process.then
...
https://juejin.cn/post/7281192416077021236?searchId=202310151059079670250A8014FC3339A1
########## 模块化相关 #########
